...
□ 4. LER o arquivo .md do agente (ex: .github/agents/frontend-specialist.md)
...
### Architecture
- **Active skills**: in `.github/skills/` — see `ARCHITECTURE.md` for full list
- **Vault skills** (~1200+): in `.copilot-vault/` — discoverable via index

### Routing Flow
1. Check if an **active skill** covers the request → use it directly
2. If not → open `.github/VAULT_INDEX.md` to find a vault skill
3. If found → read `.copilot-vault/{name}/SKILL.md`
...
> 🔴 **Leia `.github/instructions/agents.instructions.md` para a tabela Keyword→Agent completa.**
...
- Skill-level scripts: `.github/skills/<skill>/scripts/`
